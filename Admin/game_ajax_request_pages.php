...
if($action == 'insert' && !empty($_POST)){
  ...
  $CateFetch=$_POST['CateFetch'];
  $DBGNISO=$_POST['DBGNISO'];
  ...
  $upload_directory2  = "Alluploadfolder/gamedownfolder/";
 ...
 if(file_exists("Alluploadfolder/gameposter/".$_FILES['Poster']['name'])){
   ...
   echo json_encode(array('message' => '<h5> <i class="fa fa-bomb">Duplicate Image not Inserted</h5>'.$filename, 'status' => false ));
            ...
            $sql="insert into games(name,Add_Date,Views,Download_Rate,Categories1,Categories2,CateFetch,DBGNISO,Description,Poster,Downfolder,filename)values
            ('$name','$Add_Date','$Views','$Download_Rate','$Categories1','$Categories2','$CateFetch','$DBGNISO','$Description','$Poster','$upload_directory2','$filename')";

             $query_run = mysqli_query($con,$sql);

            if($query_run){
   move_uploaded_file($_FILES["Poster"]["tmp_name"], "Alluploadfolder/gameposter/".$_FILES["Poster"]["name"]);
            ...
            <tr>
               ...
               <th>Delete</th>
                    ...
                    while($row = mysqli_fetch_assoc($query_run)){
                        ...
                        $poster=$row['Poster'];
                          ...
                          $output .= "<body>
                              ...
                              <td><img src='Alluploadfolder/gameposter/$poster'width='50px'height='50px'></td>
                    ...
                    if( $action == 'update'&& !empty($_POST)){
                        ...
                        $upload_directory2  = "Alluploadfolder/gamedownfolder/".$filename_zip;
                        ...
                        $filename_zip_old =  $_POST["old_file_zip"];
                         ...
                         $query = "UPDATE games SET  name = '{$name}', Add_Date ='{$Add_Date}', Views='{$Views}',Download_Rate='{$Download_Rate}',Categories1='{$Categories1}',Categories2='{$Categories2}',CateFetch='{$CateFetch}',DBGNISO='{$DBGNISO}',Description='{$Description}',Poster='{$update_poster}',Downfolder='{$upload_directory2}',filename='{$update_filename_zip}'WHERE id ='{$game_id}'";
                         ...
                         $query_run = mysqli_query($con,$query);
                           ...
                           if($query_run){
                              ...
                              {
                               move_uploaded_file($_FILES["Poster"]["tmp_name"], "Alluploadfolder/gameposter/".$_FILES["Poster"]["name"]);
                                 unlink('Alluploadfolder/gamecposter/'.$poster_old_image);
                              }
                             
                              if($_FILES['movupbt']['name'] !='')
                              {
                               move_uploaded_file($_FILES["movupbt"]["tmp_name"], "Alluploadfolder/gamedownfolder/".$_FILES["movupbt"]["name"]);
                                 unlink('Alluploadfolder/gamedownfolder/'.$filename_mp3_old);
                              }
                ...
                $sql="SELECT * FROM games WHERE id='{$id}'";
                   ...
                   unlink("Alluploadfolder/gameposter/".$result["Poster"]);
                   unlink("Alluploadfolder/gamedownfolder/".$result["filename"]);

      
                   $sql1="DELETE FROM games WHERE id='{$id}'";
